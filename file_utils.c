#include "file_utils.h"
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "consts.h"
#include "data_table.h"

/*gets a list of file paths and returns the length of the longest file path */
/*returns 0 if the input is invalid */
size_t find_max_file_path_length(char *file_paths[], int num_files, ErrorInfo *error_info) {
    size_t max;
    size_t cur_len;
    int i;

    if (file_paths == NULL || num_files <= 0) {
        report_error(error_info, "Invalid input to find_max_file_path_length");
        return 0; 
    }

    max = 0;

    for (i = 0; i < num_files; i++) {
        if (file_paths[i] == NULL) {
            report_errorf(error_info, "file_paths[%d] is NULL", i);
        }
        else{
            cur_len = strlen(file_paths[i]);
            if (cur_len > max) {
                max = cur_len;
            }
        }
    }

    return max;
}

/*gets file path with an as extension, puts the base file path without the extension into output_name*/
/*returns 1 if successful, -1 if error*/
int extract_file_name_without_extension(const char *file_path, char *output_name, int max_file_name_len, ErrorInfo *error_info) {
    size_t file_path_len;
    size_t output_name_len;

    if (file_path == NULL|| output_name == NULL) {
        report_error(error_info, "failed to extract file name");
        return -1;
    }

    if (max_file_name_len <= 0) {
        report_error(error_info, "max_file_name_len must be greater than 0");
        return -1;
    }
    file_path_len = strlen(file_path);

    if (file_path_len <= 3){
        report_error(error_info, "file path is too short");
        return -1;
    }

    if (strcmp(file_path + file_path_len - 3, ".as") != 0) {
        report_errorf(error_info, "file path \"%s\" does not end with .as", file_path);
        return -1;
    }

    output_name_len = file_path_len - 3;

    if (output_name_len +1  > (size_t)max_file_name_len) {
        report_error(error_info, "output_name string is too small");
        return -1;
    }

    memcpy(output_name, file_path, output_name_len);
    output_name[output_name_len] = '\0'; 

    return 1;
}

/*adds extension to a file name, puts it in output_path*/
/*returns 1 if successful, -1 if error*/
int add_file_extension(char *file_name_no_extension, char *extension, char *output_path, int max_file_path_len, ErrorInfo *error_info){
    size_t file_name_len;
    size_t extension_len;

    if (file_name_no_extension == NULL || extension == NULL || output_path == NULL) {
        report_error(error_info, "Invalid input to add_file_extension");
        return -1;
    }

    if (max_file_path_len <= 0) {
        report_error(error_info, "max_file_path_len must be greater than 0");
        return -1;
    }

    file_name_len = strlen(file_name_no_extension);
    extension_len = strlen(extension);


        if (file_name_len + extension_len + 1 > (size_t)max_file_path_len) {
        report_error(error_info, "Output path buffer is too small.");
        return -1;
    }

    memcpy(output_path, file_name_no_extension, file_name_len);
    memcpy(output_path + file_name_len, extension, extension_len);
    output_path[file_name_len + extension_len] = '\0';

    return 1;
}

/*writes bytes to the .ob file*/
/*returns 1 if successful, -1 if error*/
int write_bytes_to_ob_file(FILE *ob_file, const unsigned char *image, int cur_idx, int total_size, ErrorInfo *error_info) {
    int byte_idx;
    for (byte_idx = 0; byte_idx < 4 && cur_idx + byte_idx < total_size; byte_idx++) {
        if (fprintf(ob_file, " %02X", (unsigned int)image[cur_idx + byte_idx]) < 0) {
            report_error(error_info, "Error writing to .ob file.");
            return -1;
        }
    }
    if (fprintf(ob_file, "\n") < 0) {
        report_error(error_info, "Error writing to .ob file");
        return -1;
    }
    return 1;
}


/*writes the .ob file*/
/*returns 1 if successful, -1 if error*/
int write_ob_file(FILE *ob_file, const AssemblerData *data, ErrorInfo *error_info){
    int code_size;
    int data_size;
    int code_idx, byte_idx;

    if (ob_file == NULL || data == NULL) {
        return -1; /*Error*/
    }
    code_size = data->IC - 100; /* Assuming IC starts at 100 */
    data_size = data->DC;

    if (fprintf(ob_file, "     %d %d\n", code_size, data_size) < 0) {
        report_error(error_info, "Error writing to .ob file.");
        return -1;
    }

    for (code_idx = 0; code_idx < code_size; code_idx+=4) {
        if (fprintf(ob_file, "%04d", 100 + code_idx) < 0) { /* writing the address */
            report_error(error_info, "Error writing to .ob file.");
            return -1;
        }

        if (write_bytes_to_ob_file(ob_file, data->code_image, code_idx, code_size, error_info) == -1) {
            return -1;
        }
    }


    for (byte_idx = 0; byte_idx < data_size; byte_idx+=4) {
        if (fprintf(ob_file, "%04d", 100 + code_size + byte_idx) < 0) { /* writing the address */
            report_error(error_info, "Error writing to .ob file.");
            return -1;
        }

        if (write_bytes_to_ob_file(ob_file, data->data_image, byte_idx, data_size, error_info) == -1) {
            return -1;
        }
    }

    return 1;
}

/*closes a file safly*/
/*returns 1 if succsful and -1 if error*/
int close_file(FILE *file, const char *file_name, ErrorInfo *error_info) {
    if (file == NULL){
        return 1; 
    }
    if (fclose(file) != 0) {
        report_errorf(error_info, "Failed to close file %s", file_name);
        if (file_name != NULL) {
            if (remove(file_name) != 0) {
                report_errorf(error_info, "Failed to remove file %s", file_name);
            }
            return -1;
        }
        else {
            report_error(error_info, "file_name is NULL, cannot remove file");
            return -1;
        }
    }
    
    return 1;
}

/*returns 1 if successful and -1 if error*/
int close_files(FILE *am_file, FILE *ob_file, FILE *ext_file, FILE *ent_file, const char *am_file_name, const char *ob_file_name, const char *ext_file_name, const char *ent_file_name, ErrorInfo *error_info) {
    int error_flag = 0;
    if (am_file != NULL) {
        if (close_file(am_file, am_file_name, error_info) == -1) {
            error_flag = 1;
        }
    }
    if (ob_file != NULL) {
        if (close_file(ob_file, ob_file_name, error_info) == -1) {
            error_flag = 1;
        }
    }
    if (ext_file != NULL) {
        if (close_file(ext_file, ext_file_name, error_info) == -1) {
            error_flag = 1;
        }
    }
    if (ent_file != NULL) {
        if (close_file(ent_file, ent_file_name, error_info) == -1) {
            error_flag = 1;
        }
    }
    return error_flag == 0 ? 1 : -1;
    
}


/*deletes the files*/
/*first closes the files and then deletes them*/
int remove_files(FILE *am_file, FILE *ob_file, FILE *ext_file, FILE *ent_file,
     char *am_file_name, char *ob_file_name, char *ext_file_name, char *ent_file_name, ErrorInfo *error_info) {
    int error_flag = 0;
    if (am_file != NULL) {
        if (fclose(am_file) != 0) {
            report_errorf(error_info, "Failed to close intermediate file %s", am_file_name);
            error_flag = 1;
        }
    }
    if (am_file_name != NULL) {
        remove(am_file_name);
    }
    if (ob_file != NULL) {
        if (fclose(ob_file) != 0) {
            report_errorf(error_info, "Failed to close output file %s", ob_file_name);
            error_flag = 1;
        }
    }
    if (ob_file_name != NULL) {
        remove(ob_file_name);
    }

    if (ext_file != NULL) {
        if (fclose(ext_file) != 0) {
            report_errorf(error_info, "Failed to close external file %s", ext_file_name);
            error_flag = 1;
        }
    }
    if (ext_file_name != NULL) {
        remove(ext_file_name);
    }
    if (ent_file != NULL) {
        if (fclose(ent_file) != 0) {
            report_errorf(error_info, "Failed to close entry file %s", ent_file_name);
            error_flag = 1;
        }
    }
    if (ent_file_name != NULL) {
        remove(ent_file_name);
    }
    return error_flag == 0 ? 1 : -1; /* success if no errors */
    
}


/*writes the .ext file*/
/*returns 1 if successful, -1 if error*/
int write_ext_file(FILE **ext_file, char *ext_file_name, ExternUsageNode *extern_head, ErrorInfo *error_info) {
    ExternUsageNode *current = extern_head;
    
    if (ext_file == NULL || *ext_file == NULL || ext_file_name == NULL) {
        report_error(error_info, "ext_file is NULL or ext_file_name is NULL");
        return -1;
    }

    if (extern_head == NULL) {
        if (fclose(*ext_file) != 0) {
            report_errorf(error_info, "Failed to close external file %s", ext_file_name);
            remove(ext_file_name);
            *ext_file = NULL; /* Set the pointer to NULL to indicate that the file is closed */
            return -1;
        }
        if (remove(ext_file_name) != 0) { /*removing existing file, to make sure that if the extern_head is an
        empty list there wont be any ext files from previos runs*/
            report_errorf(error_info, "Failed to remove external file %s", ext_file_name);
            *ext_file = NULL; /* Set the pointer to NULL to indicate that the file is closed */
            return -1;
        }
        *ext_file = NULL; /* Set the pointer to NULL to indicate that the file is closed */
        return 1; /* No extern symbols to write */
    }
    
    while (current != NULL) {
        if (fprintf(*ext_file, "%s %04ld\n", current->name, current->address) < 0) {
            report_errorf(error_info, "Failed to write to external file %s", ext_file_name);
            return -1;
        }
        current = current->next;
    }

    return 1; /* success */
}


/*writes the .ent file*/
/*returns 1 if successful, -1 if error*/
int write_ent_file(FILE **ent_file, char *ent_file_name, LabelNode *label_head, ErrorInfo *error_info) {
    LabelNode *current = label_head;
    int entry_exist = 0;

    if (ent_file == NULL || *ent_file == NULL ||ent_file_name == NULL) {
        report_error(error_info, "ent_file is NULL or ent_file_name is NULL");
        return -1;
    }

    if (label_head == NULL) {
        if (fclose(*ent_file) != 0) {
            report_errorf(error_info, "Failed to close entry file %s", ent_file_name);
            remove(ent_file_name);
            *ent_file = NULL; /* Set the pointer to NULL to indicate that the file is closed */
            return -1;
        }
        if (remove(ent_file_name) != 0) { /*removing existing file, to make sure that if the label_head is an
        empty list there wont be any ent files from previos runs*/
            report_errorf(error_info, "Failed to remove entry file %s", ent_file_name);
            *ent_file = NULL; /* Set the pointer to NULL to indicate that the file is closed */
            return -1;
        }
        *ent_file = NULL; /* Set the pointer to NULL to indicate that the file is closed */
        return 1; /* No entry symbols to write */
    }
    
    while (current != NULL) {
        if (current->is_entry) {
            entry_exist = 1;
            if (fprintf(*ent_file, "%s %04ld\n", current->name, current->address) < 0) {
                report_errorf(error_info, "Failed to write to entry file %s", ent_file_name);
                return -1;
            }
        }
        current = current->next;
    }

    if (entry_exist == 0) {
        /* No entry symbols to write, remove the file */
        return remove_file(ent_file, ent_file_name, error_info);
    }
    return 1; /* success */
}

/*removes the file from the system*/
/*returns 1 if successful, -1 if error*/
int remove_file(FILE **file, char *file_name, ErrorInfo *error_info) {
    if (file == NULL || *file == NULL || file_name == NULL) {
        report_error(error_info, "file is NULL or file_name is NULL");
        return -1;
    }
    if (close_file(*file, file_name, error_info) != 1) {
        report_errorf(error_info, "Failed to close file %s", file_name);
        remove(file_name);
        *file = NULL; 
        return -1;
    }
    *file = NULL; /* Set the pointer to NULL to indicate that the file is closed */

    if (remove(file_name) != 0) { 
        report_errorf(error_info, "Failed to remove file %s", file_name);
        *file = NULL; /* Set the pointer to NULL to indicate that the file is closed */
        return -1;
    }
    return 1; /* success */
}
